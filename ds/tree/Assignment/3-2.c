...
lvn *head = NULL;
lvn *leftView(treeNode *btNode, int level){
	...
	if(btNode == NULL){
		...
		if(level == nodeCount){
			struct leftViewNode *node =(lvn*) malloc(sizeof(lvn));
			node->data = btNode->data;
			printf("%d ",node->data);
			nodeCount++;
		}
		leftView(btNode->left,level+1);
		leftView(btNode->right,level+1);
	}
...
void main(){
	...
	struct leftViewNode *root = leftView(rootNode,0);
//	printTreee(root);
}
